// Mục đích: xử lý chỉ số chất lượng, import chỉ số, phân công chỉ số và các chức năng liên quan.
...
ORDER BY cs.MaChiSo";
        ...
        public AssignmentStatisticsViewModel GetStatistics()
        {
            var model = new AssignmentStatisticsViewModel();
            ...
            model.TongKhoaPhong = Convert.ToInt32(Scalar("SELECT COUNT(*) FROM dbo.KhoaPhong WHERE Used=1"));
            model.ChuaPhanCong = Convert.ToInt32(Scalar(@"
...
AND NOT EXISTS (SELECT 1 FROM dbo.PhanCongChiSo pc WHERE pc.ChiSoChatLuongId=cs.ChiSoChatLuongId AND pc.DangHoatDong=1)"));
            model.DangHoatDong = model.TongPhanCong;
            model.TamDung = Convert.ToInt32(Scalar("SELECT COUNT(*) FROM dbo.PhanCongChiSo WHERE DangHoatDong=0"));
            return model;
        }
