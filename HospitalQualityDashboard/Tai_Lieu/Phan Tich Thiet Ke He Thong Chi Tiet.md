...
flowchart TD
    ...
    subgraph Business_Layer ["Tầng Nghiệp Vụ (Service Layer)"]
        ...
        Svc["Services (AuthService, IndicatorPeriodServices, ReportDashboardServices, NotificationExportServices...)"]
    end
...
classDiagram
    ...
    class IndicatorPeriodServices {
        ...
        +PreviewAssignments(int[] indicatorIds, int[] departmentIds) List~PreviewAssignmentViewModel~
    ...
    class AssignmentController {
        -IndicatorPeriodServices _indicatorService
        +Index(int filterKhoaId, int filterChiSoId, int page) ActionResult
        +Preview(int[] indicatorIds, int[] departmentIds) JsonResult
...
File nghiệp vụ `Phân chia các chỉ số dựa theo đơn vị thu thập và tổng hợp.docx` có đủ thông tin mô tả chỉ số nhưng không có nhãn/cột riêng cho `Đơn vị tính`. Vì vậy hệ thống không thay đổi schema, mà bổ sung xử lý trong `IndicatorPeriodServices.BuildIndicatorFromRow`:
...
Script `tools/VerifyIndicatorFormulaImport.ps1` kiểm tra các case bắt buộc: alias nhãn DOCX, suy luận công thức, ưu tiên `DonViTinh` có sẵn trong file, suy luận các đơn vị tỷ số/thời gian/số lượng. Probe thực tế trên file DOCX nguồn xác nhận parser đọc được 55/55 chỉ số và không còn dòng thiếu `DonViTinh`.
