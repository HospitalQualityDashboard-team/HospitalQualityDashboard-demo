...
Da co ke hoach don dep va ghi nhan trong tai lieu. Hien trang thuc te van con mot so muc ton dong:
...
Đợt cập nhật này bổ sung bộ lọc tần suất báo cáo, cơ chế xếp loại hoàn thành tự động và các cột thống kê tiến độ nâng cao cho Dashboard Admin cùng tính năng xuất Excel.

### 29.1. Bộ lọc Tần suất trên Dashboard
- Dropdown bộ lọc tần suất (Tháng, Quý, 6 tháng, Năm) được tích hợp trực tiếp trên Dashboard Admin.
- Khi chọn lọc một tần suất cụ thể, tất cả số liệu thống kê (Tổng chỉ số, Đã nộp, Còn thiếu, Quá hạn) cùng biểu đồ Chart.js và danh sách khoa/phòng sẽ được tính toán động chỉ dựa trên các chỉ số thuộc tần suất được lọc.
- Tệp Excel xuất ra cũng đồng bộ với bộ lọc tần suất đang chọn trên giao diện.

### 29.2. Cơ chế Xếp loại Hoàn thành
- Đánh giá xếp loại tiến độ nộp báo cáo của khoa/phòng dựa trên tỷ lệ hoàn thành (số báo cáo đã gửi / tổng số chỉ số được giao):
  - **Xuất sắc**: Tỷ lệ hoàn tất >= 90%
  - **Khá**: 70% <= Tỷ lệ hoàn tất < 90%
  - **Trung bình**: 50% <= Tỷ lệ hoàn tất < 70%
  - **Yếu**: Tỷ lệ hoàn tất < 50%
  - **N/A**: Nếu tổng số chỉ số được giao bằng 0 (chưa được phân công chỉ số nào của tần suất đang xét).

### 29.3. Cột Thống Kê Nâng Cao
- Giao diện và tệp Excel bổ sung các cột:
  - *Số báo cáo lưu nháp*, *Số báo cáo còn thiếu*, *Xếp loại tổng thể*.
